...
def Pause_function(win):
    ...
    txt1 = Text(Point(400, 210), "Game has been paused , Press Anywhere To Continue Game")
    txt1.draw(win)
    click_music()                    
...
def Main():
    ...
    while(key < int(difficult/2)):
        ...
        if(counter < 2):        
            ...
            if(user_point.x < 80 and user_point.x > 15 and user_point.y < 320 and user_point.y > 250):
                    sound_button_function()
            if(user_point.x < 100 and user_point.x > 0 and user_point.y < 410 and user_point.y > 320):
                    Pause_function(win) 
            if(clicked_card > 0):
                '''
