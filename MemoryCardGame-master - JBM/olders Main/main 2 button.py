...
# Global variable to track the game state (paused or playing)
game_paused = False
...
def Main():
    ...
    play_button = Rectangle(Point(720, 10), Point(780, 40))
    play_button.setFill("green")
    play_button.draw(win)

    play_text = Text(Point(750, 25), "Play")
    play_text.setSize(14)
    play_text.setStyle('bold')
    play_text.setTextColor("white")
    play_text.draw(win)

    pause_button = Rectangle(Point(720, 50), Point(780, 80))
    pause_button.setFill("red")
    pause_button.draw(win)

    pause_text = Text(Point(750, 65), "Pause")
    pause_text.setSize(14)
    pause_text.setStyle('bold')
    pause_text.setTextColor("white")
    pause_text.draw(win)

    quit_button = Rectangle(Point(720, 90), Point(780, 120))
    quit_button.setFill("gray")
    quit_button.draw(win)

    quit_text = Text(Point(750, 105), "Quit")
    ...
    while True:
        if not game_paused:
            ...
            user_point = win.getMouse()
            if 720 <= user_point.getX() <= 780 and 10 <= user_point.getY() <= 40:
                game_paused = False
                play_button.setFill("green")
                pause_button.setFill("red")
                play_text.setText("Play")
                pause_text.setText("Pause")
            elif 720 <= user_point.getX() <= 780 and 50 <= user_point.getY() <= 80:
                game_paused = True
                play_button.setFill("red")
                pause_button.setFill("green")
                play_text.setText("Resume")
                pause_text.setText("Paused")
            elif 720 <= user_point.getX() <= 780 and 90 <= user_point.getY() <= 120:
                win.close()
                exit()
